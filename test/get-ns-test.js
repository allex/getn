...
describe('parseNs', () => {
  ...
  test('trace parseNs (k, v, o, n)', () => {
    ...
    assert.equal(model.n || 0, path.length - 1, 'invalid paths trace position')
  })
  ...
  test('trace parseNs count deeply', () => {
    ...
    assert.equal(model.k, 'o')
    assert.equal(model.n || 0, 4, 'invalid paths trace position')
  })
})
